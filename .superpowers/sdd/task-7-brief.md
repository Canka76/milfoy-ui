# Task 7 Brief: Preview 3D Viewport — PreviewRenderUtility, Camera, Render Loop
...
## Instructions
1. Create directory `Editor/Viewport` if it does not exist.
2. Create `Editor/Viewport/UIPreview3DViewport.cs` with the exact C# code specified.
3. Ensure proper formatting and LF line endings.
4. Stage with `git add Editor/Viewport/UIPreview3DViewport.cs`.
5. Commit with message: `feat(viewport): implement PreviewRenderUtility viewport with camera controls and picking`.
6. Write report to `.superpowers/sdd/task-7-report.md`.
